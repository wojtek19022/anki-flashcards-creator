...
from .constants import LOGGER, CONSOLE_USED, DEFAULT_NUM_PROC, \
                        MODEL_NAME, LANGUAGES_DECKS, FIELDS, FIELDS_EXCEL, CURR_LANG, CARD_TMPLT, DICT_LANG_SEARCH_URLS, \
                        DIKI_MAIN_URL
...
class AnkiNoteGenerator:
    def __init__(self, mw):
        ...
        self.mw = None if self.console_used else mw
        ...
        # else:
        #     with open(r"C:\Users\wojta\Desktop\test.txt","a") as file:
        #         for line in self.anki_client_desktop.get_cards_details(self.anki_client_desktop.find_all_notes()):
        #             if card != {} and card["modelName"] == self.model_name:
        #                 file.write(str(card),"\n")
        #     self.cards_in_deck = [
        #         card for card in self.anki_client_desktop.get_cards_details(self.anki_client_desktop.find_all_notes()) \
        #         if card != {} and card["modelName"] == self.model_name
        #     ]
        # self.fields = self.anki_client_console.get_fields_by_model_name(self.model_name)
        self.data = ""
        self.data_cols = []
    ...
    #     if not self.model_name in self.anki_client_console.get_models_names():
    #         logging.error(f'Model with name: {self.model_name} is not on ANKI. Try with different name')
    #         return 
        
    #     if self.current_lang not in self.anki_client_console.get_decks_and_id().keys():
